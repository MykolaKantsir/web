...
const drawingOverlayRenderer = {
    ...
    img.onload = () => {
        ...
        pdf.addImage(imageData, "PNG", offsetX, offsetY, finalWidth, finalHeight);
        const baseName = drawingData.drawing ? drawingData.drawing.replace(/\.[^.]+$/, "") : null;
        const emptyFormName = baseName ? `empty_form_${baseName}.pdf` : "empty_protocol_form.pdf";
        pdf.save(emptyFormName);
    };
    },

    renderOverlayForm(drawingData) {
        const { jsPDF } = window.jspdf;
        ...
        img.onload = () => {
            ...
            drawingData.dimensions.forEach((dim, i) => {
                ...
                // Draw rectangular frame only (no white fill — original content stays visible)
                ctx.strokeRect(x, y, w, h);

                // Draw dimension number outside the box (top-left corner)
                if (dim.dimension_number) {
                    ctx.fillStyle = "#000000";
                    ctx.font = "bold 24px Arial";
                    ctx.textAlign = "left";
                    ctx.textBaseline = "bottom";
            ...
            pdf.addImage(imageData, "PNG", offsetX, offsetY, finalWidth, finalHeight);
            const baseName = drawingData.drawing ? drawingData.drawing.replace(/\.[^.]+$/, "") : null;
            const overlayName = baseName ? `overlay_${baseName}.pdf` : "template_overlay.pdf";
            pdf.save(overlayName);
        };
    },

    savePDF(pdf) {
        const drawingName = this.pages?.[0]?.protocol?.drawing;
        const baseName = drawingName ? drawingName.replace(/\.[^.]+$/, "") : null;
        const filename = baseName ? `overlay_${baseName}.pdf` : "overlay_protocols.pdf";
        pdf.save(filename);
    }
};
